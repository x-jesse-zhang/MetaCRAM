...
#dev: McuiucOutput
my $output_dir;
#dev: default '/home/user/kraken/DB/minikraken_20141208' for VM
my $kraken_db_location;
#dev: default '/opt/kraken'
my $kraken_dir;
#dev: default '/home/user/genus_db/genus_db2/'
...
if (! defined $kraken_db_location) {
	...
	$kraken_db_location="/home/user/kraken/DB/minikraken_20141208";
}
...
if (! defined $genus_db_location) {
	...
	$genus_db_location='/home/user/genus_db/genus_db2/';
}
...
sub run_kraken {
	...
	system("kraken-mpa-report --db $kraken_db_location $kraken_output_dir/KrakenOutput>$kraken_output_dir/KrakenOutputReport");
...
sub run_bowtie {
	...
	# DEBUG: disable this
	system("bowtie2-build -f $fna $bowtieOut>$BTbuildLog");
	print("Done.\n");

	print("Running bowtie2. ");
	#REDIRECT OUTPUT TO STDOUT
	# DEBUG: disable this
	system("bowtie2 --reorder --threads $threads --mm -x $bowtieOut -1 $bowtieInput1 -2 $bowtieInput2 -S $SAMfile -f --no-hd --no-sq>$BT2Log");
	print("Done.\n");
...
sub run_BLAST {
	...
	# } else {
		# DEBUG: disable this
		system("blastn -db nt -query $output_dir_temp/assembled/round$roundOfUnaligned/contig.fa -num_threads $threads -out $blast_output_dir/round$roundOfUnaligned/blastOUT -max_target_seqs 1 -outfmt \"6 qseqid sseqid sgi sacc stitle evalue\"");
	# }
...
sub filter_BLAST {
	...
	if ($idba_splited) {
		for (my $i = 1; $i <= $splitCount; $i++) {
			...
			system("blastdbcmd -db nt -entry_batch $blast_output_dir/round$roundOfUnaligned/part$i/blastOUT_parsed.txt -out $blast_output_dir/round$roundOfUnaligned/part$i/out.fna -outfmt %f");
		}
		...
		system("blastdbcmd -db nt -entry_batch $blast_output_dir/round$roundOfUnaligned/blastOUT_parsed.txt -out $blast_output_dir/round$roundOfUnaligned/out.fna -outfmt %f");
		print "Done.\n";
	}
...
sub runCompression {
	...
	if ($roundOfUnaligned == $maxRounds) {
		...
		system("MFCompressC $cram_dir/unaligned$roundOfUnaligned");
