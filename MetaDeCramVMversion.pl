...
my $inputDir;
my $blast_location = "/home/user/blast/ncbi-blast-2.2.31+/bin/";
my %fnaHash;
...
for (my $i = 1; $i <= $maxRounds; $i++) {
	...
	if ($i == 1) {
		...
		# chdir "/shared3";
		system("blastdbcmd -db nt -entry_batch $currentDir/blastOUT_parsed.txt -out $currentDir/out.fna -outfmt %f");
		chdir $currentDir;
...
# if ($DEBUGLOGGING) {
	system("perl ~/MetaCRAMrun/OUTsorter.pl $inputDir/OUT $inputDir/sortedOUT");
	system("perl ~/MetaCRAMrun/OUTsorter.pl $inputDir/OUT2 $inputDir/sortedOUT2");
# }
